...
abstract class REST_Controller extends CI_Controller
{
	...
	public function generateBillingRef()
	{
	...
	public function __construct($config = 'rest')
	{
		...
		$this->_supported_formats = array_intersect_key(
			$this->_supported_formats,
			array_flip($supported_formats)
		);
		...
		if ($this->request->format && $this->request->body) {
			$this->request->body = $this->format
				->factory($this->request->body, $this->request->format)
				->to_array();
		...
		if (
			...
			$this->config->item('rest_ajax_only')
		) {
			...
			$this->response(
				[
					...
					$this->config->item(
						'rest_message_field_name'
					) => $this->lang->line('text_rest_ajax_only'),
				],
		...
		if (
			...
			(!($this->config->item('rest_enable_keys') &&
				$this->_allow === true) ||
				($this->config->item('allow_auth_and_keys') === true &&
					$this->_allow === true))
		) {
	...
	public function _remap($object_called, $arguments = [])
	{
		...
		if (
			$this->config->item('force_https') &&
			$this->request->ssl === false
		) {
			$this->response(
				[
					$this->config->item('rest_status_field_name') => false,
					$this->config->item(
						'rest_message_field_name'
					) => $this->lang->line('text_rest_unsupported'),
				],
				self::HTTP_FORBIDDEN
			);
		}
		...
		$log_method = !(isset($this->methods[$controller_method]['log']) &&
			$this->methods[$controller_method]['log'] === false);

		// Use keys for this method?
		$use_key = !(isset($this->methods[$controller_method]['key']) &&
			$this->methods[$controller_method]['key'] === false);
		...
		if (
			...
			$this->response(
				[
					'status' => 'failure',
					'message' => sprintf(
						$this->lang->line('text_rest_invalid_api_key'),
						$this->rest->key
					),
				],
				self::HTTP_FORBIDDEN
			...
			$this->response(
				[
					...
					'message' => $this->lang->line(
						'text_rest_api_key_unauthorized'
					),
				],
		...
		if (!method_exists($this, $controller_method)) {
			$this->response(
				[
					'status' => 'failure',
					'message' => $controller_method . ' ' . $this->lang->line('text_rest_unknown_method'),
				],
		...
		if (
			...
			// Check the limit
			if (
				$this->config->item('rest_enable_limits') &&
				$this->_check_limit($controller_method) === false
			) {
				$response = [
					'status' => 'failure',
					'message' => $this->lang->line(
			...
			$level = isset($this->methods[$controller_method]['level'])
				? $this->methods[$controller_method]['level']
				: 0;
		...
		elseif (
			$this->config->item('rest_limits_method') == 'IP_ADDRESS' &&
			$this->config->item('rest_enable_limits') &&
			$this->_check_limit($controller_method) === false
		) {
			$response = [
				'status' => 'failure',
				'message' => $this->lang->line(
					'text_rest_ip_address_time_limit'
				),
			];
			$this->response($response, self::HTTP_UNAUTHORIZED);
		}
	...
	protected function _detect_output_format()
	{
		...
		if (
			$this->config->item('rest_ignore_http_accept') === false &&
			$http_accept !== null
		) {
	...
	protected function _detect_method()
	{
		...
		return in_array($method, $this->allowed_http_methods) &&
			method_exists($this, '_parse_' . $method)
			? $method
			: 'get';
	}
	...
	protected function _detect_api_key()
	{
		...
		$key_name =
			'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));
		...
		if (
			$key = isset($this->_args[$api_key_variable])
	...
	protected function _check_limit($controller_method)
	{
		...
		switch ($this->config->item('rest_limits_method')) {
			...
			default:
				...
				if (
					strpos(
						strrev($limited_uri),
						strrev($this->response->format)
					) === 0
				) {
					$limited_uri = substr(
						$limited_uri,
						0,
						-strlen($this->response->format) - 1
					);
				}
				$limited_uri =
					'uri:' . $limited_uri . ':' . $this->request->method; // It's good to differentiate GET from PUT
				break;
		}
		...
		$time_limit = isset($this->methods[$controller_method]['time'])
			? $this->methods[$controller_method]['time']
			: 3600; // 3600 = 60 * 60
	...
	protected function _auth_override_check()
	{
		...
		$auth_override_class_method = $this->config->item(
		...
		if (!empty($auth_override_class_method)) {
			...
			if (
				...
				if (
					$auth_override_class_method[$this->router->class][$this->router->method] === 'none'
				) {
					return true;
				}
				...
				// Basic auth override found, prepare basic
				if (
					$auth_override_class_method[$this->router->class][$this->router->method] === 'basic'
				) {
					$this->_prepare_basic_auth();
				...
				// Digest auth override found, prepare digest
				if (
					$auth_override_class_method[$this->router->class][$this->router->method] === 'digest'
				) {
					$this->_prepare_digest_auth();
				...
				// Session auth override found, check session
				if (
					$auth_override_class_method[$this->router->class][$this->router->method] === 'session'
				) {
					$this->_check_php_session();
				...
				// JWT auth override found, check jwt
				if (
					$auth_override_class_method[$this->router->class][$this->router->method] === 'jwt'
				) {
					$this->_check_jwt();
				...
				// Whitelist auth override found, check client's ip against config whitelist
				if (
					$auth_override_class_method[$this->router->class][$this->router->method] === 'whitelist'
				) {
					$this->_check_whitelist_auth();
		...
		$auth_override_class_method_http = $this->config->item(
		...
		if (!empty($auth_override_class_method_http)) {
			// check for wildcard flag for rules for classes
			if (
				!empty($auth_override_class_method_http[$this->router->class]['*'][$this->request->method])
			) {
				// None auth override found, prepare nothing but send back a TRUE override flag
				if (
					$auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'none'
				) {
					return true;
				}
				...
				// Basic auth override found, prepare basic
				if (
					$auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'basic'
				) {
					$this->_prepare_basic_auth();
				...
				// Digest auth override found, prepare digest
				if (
					$auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'digest'
				) {
					$this->_prepare_digest_auth();
				...
				// Session auth override found, check session
				if (
					$auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'session'
				) {
					$this->_check_php_session();
				...
				// JWT auth override found, check jwt
				if (
					$auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'jwt'
				) {
					$this->_check_jwt();
				...
				// Whitelist auth override found, check client's ip against config whitelist
				if (
					$auth_override_class_method_http[$this->router->class]['*'][$this->request->method] === 'whitelist'
				) {
					$this->_check_whitelist_auth();
			...
			if (
				...
				// None auth override found, prepare nothing but send back a TRUE override flag
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'none'
				) {
					return true;
				}
				...
				// Basic auth override found, prepare basic
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'basic'
				) {
					$this->_prepare_basic_auth();
				...
				// Digest auth override found, prepare digest
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'digest'
				) {
					$this->_prepare_digest_auth();
				...
				// Session auth override found, check session
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'session'
				) {
					$this->_check_php_session();
				...
				// JWT auth override found, check jwt
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'jwt'
				) {
					$this->_check_jwt();
				...
				// Whitelist auth override found, check client's ip against config whitelist
				if (
					$auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] === 'whitelist'
				) {
					$this->_check_whitelist_auth();
	...
	protected function _parse_head()
	{
		...
		parse_str(
			parse_url($this->input->server('REQUEST_URI'), PHP_URL_QUERY),
			$head
		);
	...
	public function get($key = null, $xss_clean = null)
	{
		...
		return isset($this->_get_args[$key])
	...
	public function options($key = null, $xss_clean = null)
	{
		...
		return isset($this->_options_args[$key])
			? $this->_xss_clean($this->_options_args[$key], $xss_clean)
			: null;
	}
	...
	public function head($key = null, $xss_clean = null)
	{
		...
		return isset($this->_head_args[$key])
			? $this->_xss_clean($this->_head_args[$key], $xss_clean)
			: null;
	}
	...
	public function post($key = null, $xss_clean = null)
	{
		...
		return isset($this->_post_args[$key])
			? $this->_xss_clean($this->_post_args[$key], $xss_clean)
			: null;
	}
	...
	public function put($key = null, $xss_clean = null)
	{
		...
		return isset($this->_put_args[$key])
	...
	public function delete($key = null, $xss_clean = null)
	{
		...
		return isset($this->_delete_args[$key])
	...
	public function patch($key = null, $xss_clean = null)
	{
		...
		return isset($this->_patch_args[$key])
			? $this->_xss_clean($this->_patch_args[$key], $xss_clean)
			: null;
	}
	...
	public function query($key = null, $xss_clean = null)
	{
		...
		return isset($this->_query_args[$key])
			? $this->_xss_clean($this->_query_args[$key], $xss_clean)
			: null;
	}
	...
	protected function _perform_ldap_auth($username = '', $password = null)
	{
		...
		log_message(
			'debug',
			'LDAP Auth: Connect to ' .
				(isset($ldaphost) ? $ldaphost : '[ldap not configured]')
		);
		...
		if ($ldapconn) {
			log_message(
				'debug',
				'Setting timeout to ' . $ldap['timeout'] . ' seconds'
			);
		...
		if (
			...
			log_message(
				'error',
				'LDAP Auth: User ' . $username . ' not found in search'
			);
			return false;
		}
		...
		if (($entry_id = ldap_first_entry($ldapconn, $res_id)) === false) {
			log_message(
				'error',
				'LDAP Auth: Failure, entry of search result could not be fetched'
			);
			return false;
		}
		...
		if (($user_dn = ldap_get_dn($ldapconn, $entry_id)) === false) {
			log_message(
				'error',
				'LDAP Auth: Failure, user-dn could not be fetched'
			);
			return false;
		}
		...
		if (($link_id = ldap_bind($ldapconn, $user_dn, $password)) === false) {
			log_message(
				'error',
				'LDAP Auth: Failure, username/password did not match: ' .
					$user_dn
			);
			return false;
		}
		...
		log_message(
			'debug',
			'LDAP Auth: Success ' . $user_dn . ' authenticated successfully'
		);
	...
	protected function _perform_library_auth($username = '', $password = null)
	{
		...
		$auth_library_class = strtolower(
			$this->config->item('auth_library_class')
		);
		$auth_library_function = strtolower(
			$this->config->item('auth_library_function')
		);

		if (empty($auth_library_class)) {
			log_message(
				'debug',
				'Library Auth: Failure, empty auth_library_class'
			);
			return false;
		}
		...
		if (empty($auth_library_function)) {
			log_message(
				'debug',
				'Library Auth: Failure, empty auth_library_function'
			);
			return false;
		}

		if (
			is_callable([$auth_library_class, $auth_library_function]) === false
		) {
			$this->load->library($auth_library_class);
		}

		return $this->{$auth_library_class}->$auth_library_function(
			$username,
			$password
		);
	}
	...
	protected function _check_login($username = null, $password = false)
	{
		...
		if ($auth_source === 'ldap') {
			log_message(
				'debug',
				"Performing LDAP authentication for $username"
			);
		...
		if ($auth_source === 'library') {
			log_message(
				'debug',
				"Performing Library authentication for $username"
			);
	...
	protected function _check_jwt()
	{
		...
		if (!empty($authorization)) {
			if (preg_match('/Bearer\s(\S+)/', $authorization, $matches)) {
				$bearer = $matches[1];
				try {
					$decoded = JWT::decode($bearer, $this->jwt_secret_key, [
						'HS256',
					]);
				} catch (Exception $e) {
					$this->response(
		...
		if (!$this->session->userdata($key)) {
			...
			$this->response(
				[
					...
					$this->config->item(
						'rest_message_field_name'
					) => $this->lang->line('text_rest_unauthorized'),
				],
				self::HTTP_UNAUTHORIZED
			);
		}
		...
		if ($username !== null) {
			...
			if (strpos(strtolower($http_auth), 'basic') === 0) {
				...
				list($username, $password) = explode(
					':',
					base64_decode(
						substr($this->input->server('HTTP_AUTHORIZATION'), 6)
					)
				);
			}
		}
	...
	protected function _prepare_digest_auth()
	{
		...
		if (
			array_key_exists('username', $digest) === false ||
			$username === false
		) {
			$this->_force_login($unique_id);
		}
		...
		if (strcasecmp($digest['response'], $valid_response) !== 0) {
			...
			$this->response(
				[
					...
					$this->config->item(
						'rest_message_field_name'
					) => $this->lang->line('text_rest_invalid_credentials'),
				],
				self::HTTP_UNAUTHORIZED
			);
		}
		...
		if (preg_match($pattern, $this->config->item('rest_ip_blacklist'))) {
			...
			$this->response(
				[
					...
					$this->config->item(
						'rest_message_field_name'
					) => $this->lang->line('text_rest_ip_denied'),
				],
				self::HTTP_UNAUTHORIZED
			);
		}
		...
		if (in_array($this->input->ip_address(), $whitelist) === false) {
			$this->response(
				[
					...
					$this->config->item(
						'rest_message_field_name'
					) => $this->lang->line('text_rest_ip_unauthorized'),
				],
				self::HTTP_UNAUTHORIZED
			);
		}
	...
	protected function _check_cors()
	{
		...
		$allowed_headers = implode(
			' ,',
			$this->config->item('allowed_cors_headers')
		);
		$allowed_methods = implode(
			' ,',
			$this->config->item('allowed_cors_methods')
		);
		...
		if ($this->config->item('allow_any_cors_domain') === true) {
			...
			if (
				in_array($origin, $this->config->item('allowed_cors_origins'))
			) {
	...
	protected function runFormValidation($form_ref)
	{
		if ($this->form_validation->run($form_ref) == false) {
			...
			$this->response($result, self::HTTP_UNPROCESSABLE_ENTITY);
		}
	}

	protected function allowWithRoles($roles = [])
	{
		$hasRole = false;
		$user = $this->getRequestUser();
		if ($user) {
			$hasRole = (array_search((int)$user->role, $roles) !== false);
		}

		if (!$hasRole) {
			$status = 'failed';
			$message = 'Access denied due lack of required permissions.';
			$this->response(compact('status', 'message'), self::HTTP_FORBIDDEN);
			return;
		}
	...
	{
		$isAdmin = !is_null($user = $this->getRequestUser());

		if ($isAdmin) {
			$isAdmin = (int)$user->role == 1 || (int)$user->role == 3 || (int)$user->role == 5;
		}
	...
	protected function allowOnlyVendor()
	{
		$isVendor = !is_null($user = $this->getRequestUser()) && (int) $user->role === 2;
		if (!$isVendor) {
			$result['status'] = 'failure';
			$result['message'] = 'Access denied. Vendor role required.';
			$this->response($result, self::HTTP_FORBIDDEN);
			return;
		}
		return $user;
	}
	...
	protected function getRequestUser($skipCheck = false)
	{
		try {
		 
			$decoded = $this->jwt_decode($this->jwt_token());
 
			$user = $this->User_model->getUserById($decoded['uid']);
			...
			if (!$user && !$skipCheck) {
				$this->response(
					[
						'status' => 'failure',
						'message' => 'The token user does not exist in the system',
					],
					self::HTTP_UNAUTHORIZED
				);
			}
	...
	protected function checkUserExists($userId)
	{
		...
		if (is_null($user)) {
			...
			$this->response($result, self::HTTP_NOT_FOUND);
			return;
		}
		return $user;
	}

	protected function pruneUser($data)
	{
	...
	protected function HttpInternalServerError($message = null, $error = null)
	{
		$result['status'] = 'failure';
		$result['message'] = $message ? $message : 'Internal server error occured. Please retry.';

		if ($error) {
			$result['error'] = $error;
		}

		$this->response($result, self::HTTP_INTERNAL_SERVER_ERROR);
		return;
	}

	protected function logActivity($module, $action, $description = "", $userId = null)
	{
		
		if (!$userId) {
			$user = $this->getRequestUser(true); 
			$userId = !!$user ? $user->id : null;
		}
 
		$ip = $this->input->ip_address();

		return $this->AuditLogModel->AuditLogModel->insertActivityLog($userId, $action, compact('module', 'ip', 'description'));
	}
}
